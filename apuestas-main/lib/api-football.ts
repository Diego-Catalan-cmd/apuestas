...
import { MatchData, OddsData, LineupsData, InjuriesData, HistoricalStats, TeamAverages } from "./types";
...
export async function searchMatch(teamA: string, teamB: string): Promise<MatchData> {
  try {
    ...
    const historicalStats = await getHistoricalStats(
      targetEvent.homeTeam?.id,
      targetEvent.awayTeam?.id
    );
  ...
  async function getHistoricalStats(homeTeamId?: number, awayTeamId?: number): Promise<HistoricalStats> {
    const results = await Promise.allSettled(
      [homeTeamId, awayTeamId].map((teamId) =>
        teamId
          ? apiClient.get(`/team/${teamId}/events/last/0`)
          : Promise.reject(new Error("No se encontró el ID del equipo"))
      )
    );

    const teamAverages = results.map((result) =>
      result.status === "fulfilled"
        ? aggregateTeamAverages(result.value.data?.events || [])
        : emptyTeamAverages()
    );

    return {
      sampleSize: Math.max(
        ...results.map((result) => result.status === "fulfilled" ? result.value.data?.events?.length || 0 : 0)
      ),
      homeTeam: teamAverages[0] || emptyTeamAverages(),
      awayTeam: teamAverages[1] || emptyTeamAverages(),
    };
  }

  function emptyTeamAverages(): TeamAverages {
    return { goals: null, corners: null, shotsOnTarget: null, cards: null, fouls: null };
  }

  function aggregateTeamAverages(events: any[]): TeamAverages {
    const values = (key: string): number[] => events
      .map((event) => Number(event?.statistics?.[key] ?? event?.[key]))
      .filter((value) => Number.isFinite(value));
    const average = (key: string): number | null => {
      const entries = values(key);
      return entries.length ? Number((entries.reduce((sum, value) => sum + value, 0) / entries.length).toFixed(2)) : null;
    };

    return {
      goals: average("goals"),
      corners: average("corners"),
      shotsOnTarget: average("shotsOnTarget"),
      cards: average("cards"),
      fouls: average("fouls"),
    };
  }
...
function getFallbackMatchData(teamA: string, teamB: string, date: string): MatchData {
  ...
  return {
    ...
    historicalStats: {
      sampleSize: 0,
      homeTeam: { goals: null, corners: null, shotsOnTarget: null, cards: null, fouls: null },
      awayTeam: { goals: null, corners: null, shotsOnTarget: null, cards: null, fouls: null },
    },
  };
}
